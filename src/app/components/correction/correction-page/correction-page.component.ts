...
export class CorrectionPageComponent implements OnInit, OnDestroy {
  ...
  constructor( public config: NgbModalConfig, public confi: ConfigurationService, private sseClient: SseClient,
               ...
               private modalService: NgbModal, public auth: AuthService, public commondata: CommonDataService,
               public navServices: NavService, public common: CommonService, public route: Router,
  ...
  matchcase(val, match) {
    ...
    // let trimVal = val ? val.trim() : '';
    // console.log(trimVal,'treim val');
    // let splitVal  = trimVal != '' ? trimVal.split(' ') : '';
    // console.log(splitVal,'splitVal val');
    // let splitValJoin  = splitVal != '' ? splitVal.join('') : '';
    // console.log(splitValJoin, 'splitValJoin');
  ...
  sseClientService(bodyValue) {
    ...
    this.subs.push(this.sseClient.stream(sseUrl, { keepAlive: true, reconnectionDelay: 3_000,
      responseType: 'event' }, {body: bodyValue  }, 'POST').subscribe((event) => {
      if (event.type === 'error') {
  ...
  answerListSuccess(successData, id) {
    if (successData.IsSuccess) {
      ...
      if (pdfpath.length != 0) {
        ...
        this.dragQuestion = [...this.studentAnswer.question_annotation];if (this.studentAnswer.answer_sheet_annotation.length != 0) {
        ...
        this.pageType = 2;
        if (this.studentAnswer.status == '4' || this.studentAnswer.status == '2') {
